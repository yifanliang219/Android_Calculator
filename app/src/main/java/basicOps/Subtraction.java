...
public class Subtraction {
    ...
    private static Complex complex(Complex a, Complex b) {
        return new Complex(Subtraction.real(a.re, b.re), Subtraction.real(a.im, b.im));
    }

    private static Complex complex(Complex a, Real b) {
        return new Complex(Subtraction.real(a.re, b), a.im);
    }

    private static Complex complex(Real a, Complex b) {
        return new Complex(Subtraction.real(a, b.re), b.im.negate());
    }
    ...
    public static Real real(Real r1, Real r2) {
        ...
        if (r1 instanceof IntFraction && r2 instanceof IntFraction) {
            ...
            if (Collections.max(ints).doubleValue() > (Math.pow(10, 5))) {
                result = new Decimal(a.value().subtract(b.value(), new MathContext(64, RoundingMode.HALF_UP)));
            } else
                result = new IntFraction(a.numerator.multiply(b.denominator).subtract(b.numerator.multiply(a.denominator)), a.denominator.multiply(b.denominator));
            ...
            } else result = new Decimal(r1.value().subtract(r2.value(), new MathContext(64, RoundingMode.HALF_UP)));
        } else if (r1 instanceof Decimal && r2 instanceof IntFraction) {
            result = real(r2, r1).negate();
        } else result = new Decimal(r1.value().subtract(r2.value(), new MathContext(64, RoundingMode.HALF_UP)));
